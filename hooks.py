...
def _plain_description(text, fallback='', max_len=300) -> str:
    """Collapse a (possibly markdown) excerpt into a single-line description for <meta> tags."""
...
def on_env(env, config, **kwargs):
    ...
    env.filters['plain_description'] = _plain_description
    try:
        result = subprocess.run(
...
def on_page_content(html, page, config, **kwargs):
    ...
    badge = (f'<span class="badge badge--{post_type}">{label}</span>'
             if label else '')
    date_el = f'<span class="post-date">{date}</span>' if date else ''
    
    header = f'<div class="post-meta">{badge}{date_el}</div>\n'
    return header + html


def on_post_page(output, page, config, **kwargs):
    """Google analytics script injection"""
    gtag_id = "G-2Q3X7BV1P2" 
    ...
    # Se inyecta justo antes de cerrar la etiqueta </head> de cualquier página HTML generada
    if "</head>" in output:
        return output.replace("</head>", f"{gtag_script}\n</head>", 1)
