...
2. **Platform independence**: XML is based on plain text files which allows any operating system to support it. This make it very convenient for cross-platform solutions in which the data must be accessed from different sources. Additionally, many programing languages support the XML format so the data can be transformed, edited and processed internally by the core of the applications.
